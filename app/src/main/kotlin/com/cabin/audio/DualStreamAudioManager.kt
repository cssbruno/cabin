...
class DualStreamAudioManager(
    ...
    private var releasing = false // guarded by lock; prevents initialization during teardown
        ...
        return when (audioType) {
            AudioStreamType.NAVIGATION -> {
                ...
                synchronized(lock) {
                    ...
                    // Coordinate producer activation with STOP and the playback pass.
                    ...
                    if (targetSlot.prefillSinceMs < 0) targetSlot.prefillSinceMs = SystemClock.elapsedRealtime()
    ...
    /** App-level gains multiply existing ducking; calls/assistant/alerts remain untouched. */
    fun setUserGains(media: Float, navigation: Float) {
        synchronized(lock) {
            ...
            navTrack?.let { track ->
                if (track.playState == AudioTrack.PLAYSTATE_PLAYING) {
                    ...
                    if (playDuration < minNavPlayDurationMs && bufferLevel > 50) {
                        val remainingMs = (minNavPlayDurationMs - playDuration).coerceAtLeast(1)
                        log(
                            "[NAV_STOP] Deferring premature stop by ${remainingMs}ms " +
                                "(played=${playDuration}ms, buffer=${bufferLevel}ms)",
                        )
                        focusHandler.removeCallbacks(navCompletionRunnable)
                        focusHandler.postDelayed(navCompletionRunnable, remainingMs)
                        return
                    }
     ...
     */
    fun stopSiriTrack() {
        synchronized(lock) { stopPurposeSlot(siriSlot) }
    }

    /** Stop call playback atomically with the worker's prefill/write/play pass. */
    fun stopPhoneCallTrack() {
        synchronized(lock) { stopPurposeSlot(phoneCallSlot) }
    }

    /** Start a new alert without replaying an unfinished preceding alert. */
    fun onAlertStarted() {
        synchronized(lock) { stopPurposeSlot(alertSlot) }
    }

    /** Finish queued PCM, including clips shorter than the normal prefill threshold. */
    fun onAlertStopped() {
        synchronized(lock) {
            val slot = alertSlot
            if (slot == null || slot.track == null ||
                (slot.buffer.availableForRead() == 0 && slot.residualCount == 0)
            ) {
                finishAlert(slot)
            } else {
                slot.ending = true
    ...
    /** Caller holds lock, shared with every purpose-slot write and play operation. */
    private fun stopPurposeSlot(slot: PurposeSlot?) {
        slot ?: return
        try {
            slot.track?.let { track ->
                if (track.playState == AudioTrack.PLAYSTATE_PLAYING) track.pause()
                // Also flush STOPPED/PAUSED tracks: they can contain preloaded PCM.
                track.flush()
            }
        } catch (e: Exception) {
            log("[AUDIO] ${slot.purpose} stop failed: ${e.message}")
        }
        clearSlotPlayback(slot)
    }

    private fun clearSlotPlayback(slot: PurposeSlot) {
        slot.started = false
        slot.pendingPlay = false
        slot.prefillSinceMs = -1
        slot.ending = false
        slot.idleSince = 0
    ...
    /** Streaming stop lets already submitted PCM finish; a new alert flushes it. */
    private fun finishAlert(slot: PurposeSlot?) {
        if (slot != null) {
            try {
                if (slot.track?.playState == AudioTrack.PLAYSTATE_PLAYING) slot.track?.stop()
            } catch (e: Exception) {
                log("[AUDIO] Alert completion failed: ${e.message}")
            }
            clearSlotPlayback(slot)
        }
        onPurposeEnded(StreamPurpose.ALERT)
    }
    ...
    fun release() {
        ...
        // The navigation writer takes lock; never join it while holding that lock.
        thread?.interrupt()
        try {
    ...
    private fun createPurposeSlot(
        ...
        return PurposeSlot(
            ...
            silenceBuffer = ByteArray(format.sampleRate * format.channelCount * 2 / 100),
        )
    }
    ...
    private fun ensureSlotFormat(
        ...
        synchronized(lock) {
            ...
            if (slot.format != incomingFormat) {
                ...
                slot.silenceBuffer =
                    ByteArray(incomingFormat.sampleRate * incomingFormat.channelCount * 2 / 100)
    ...
    private fun ensureNavTrack(decodeType: Int) {
        ...
        synchronized(lock) {
            ...
            // Rearm paused or never-started tracks after a prompt completes.
            navTrack?.let { track ->
                if (track.playState != AudioTrack.PLAYSTATE_PLAYING && navFormat == format && !navPendingPlay) {
                    val discardedMs = navBuffer?.fillLevelMs() ?: 0
                    track.flush()
                    navBuffer?.clear()
                    playbackThread?.clearNavResidual()
                    ...
                    log("[AUDIO] Rearmed nav track with flush — discarded=${discardedMs}ms (same format ${format.sampleRate}Hz)")
                    return
                }
            }

            if (navFormat != format || navTrack == null) {
    ...
    private inner class AudioPlaybackThread : Thread("AudioPlayback") {
        ...
        /** Called under lock together with pause/flush or track replacement. */
        fun clearNavResidual() {
            navResidualOffset = 0
            navResidualCount = 0
        }
            ...
            while (isRunning.get() && !isInterrupted) {
                try {
                    ...
                    if (now - lastStatsTime >= statsInterval) {
                        ...
                        val slotsSnapshot = synchronized(lock) {
                            listOfNotNull(mediaSlot, siriSlot, phoneCallSlot, alertSlot)
                        }
                        var mOvf = 0
                        var mUrun = 0
        ...
        /** A bounded pass of NON_BLOCKING writes, atomic against stop/format changes. */
        private fun processPurposeSlots() {
            synchronized(lock) {
                ...
                for (slot in slotsSnapshot) {
                    try {
                        ...
                        if (!slot.started) {
                            val shortAlertReady = slot.purpose == StreamPurpose.ALERT &&
                                buffer.availableForRead() > 0 && (slot.ending ||
                                (slot.prefillSinceMs >= 0 && SystemClock.elapsedRealtime() - slot.prefillSinceMs >= prefillThresholdMs))
                            if (currentFillMs < prefillThresholdMs && !shortAlertReady) continue
                            slot.started = true
                            if (slot.pendingPlay) {
                                var preloadRemaining = slot.tempBuffer.size
                                while (buffer.availableForRead() > 0 && preloadRemaining > 0) {
                                    ...
                                    val bytesRead =
                                        buffer.read(
                                            slot.tempBuffer,
                                            0,
                                            minOf(avail, preloadRemaining),
                                        )
                                    if (bytesRead <= 0) break
                                    preloadRemaining -= bytesRead
                                    val written =
                                        track.write(
                                            slot.tempBuffer,
                                            0,
                        ...
                        // Alert tails can be shorter than one millisecond. Use bytes so
                        // duration rounding cannot leave PCM and focus pending forever.
                        val bufferEmpty = if (slot.purpose == StreamPurpose.ALERT) {
                            buffer.availableForRead() == 0
                        } else {
                            currentFillMs <= minBufferLevelMs
                        }
                        // Skip new reads while residual pending or buffer empty.
                        if (slot.residualCount > 0 || bufferEmpty) {
                            if (track.playState == AudioTrack.PLAYSTATE_PLAYING &&
                                bufferEmpty && slot.residualCount == 0
                            ) {
                                val now = System.currentTimeMillis()
                                if (slot.idleSince == 0L) {
                                    ...
                                    if (slot.purpose == StreamPurpose.MEDIA ||
                                        ...
                                        // The playback loop runs every 5ms. Write one 5ms
                                        // frame-aligned silence slice, not the full 10ms backing
                                        // buffer, otherwise AudioTrack advances at 2x realtime.
                                        val frameSize = slot.format.channelCount * 2
                                        val silenceBytes =
                                            ((slot.format.sampleRate * frameSize * 5 / 1000) / frameSize) * frameSize
                                        val silenceWritten = track.write(
                                            slot.silenceBuffer,
                                            0,
                                            minOf(silenceBytes, slot.silenceBuffer.size),
                                            AudioTrack.WRITE_NON_BLOCKING,
                                        )
                                        if (silenceWritten < 0) {
                                            handleTrackError(slot.purpose.name, silenceWritten, slot)
                                        }
                                    } else {
                        ...
                        if (available > 0) {
                            ...
                            val toRead =
                                minOf(
                                    ...
                                    if (slot.purpose == StreamPurpose.ALERT) available else maxReadableBytes.coerceAtLeast(0),
                                )
                        ...
                        if (slot.ending && slot.buffer.availableForRead() == 0 && slot.residualCount == 0) {
                            finishAlert(slot)
                        }
                    }
                }
        ...
        // Keep residual bytes, track writes, and prompt flushes in one critical section.
        ...
        private fun handleTrackError(
            ...
            when (errorCode) {
                ...
                -> {
                    log("[AUDIO] $streamType AudioTrack unusable ($errorCode), releasing for recreation")
                    synchronized(lock) {
                        if (slot != null) {
