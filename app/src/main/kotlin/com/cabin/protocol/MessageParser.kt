...
import com.cabin.audio.AudioFormats
...
object MessageParser {
    ...
    fun parseHeader(data: ByteArray): MessageHeader {
        ...
        // The complement protects the frame boundary, not merely known message IDs.
        // Validate it for unknown/future types too so a corrupt type cannot make us
        // consume an arbitrary payload length from the USB byte stream.
        val typeCheck = buffer.int
        val expectedCheck = typeInt.inv()
 ...
 * Consumers ([CabinManager.handleMessage]) use it as a streaming-liveness
 ...
 * this class stores only the encrypted payload size, and [CabinManager.handleMessage]
