...
import com.cabin.platform.TeyesClusterMediaBridge
...
private const val TAG = "CABIN_MEDIA"
 ...
 *
 * PURPOSE
 * Exposes the Cabin app as a selectable media source on Android Automotive OS (AAOS)
 ...
 * ARCHITECTURE
 * ```
 * CarPlay/AA projection → USB adapter → CabinManager.processMediaMetadata
 *                                                      │
 *                                                      ▼
 *                                       MediaSessionManager
 *                                                      │
 ...
 *                                     (CabinManager.sendKey(CommandMapping))
 * ```
 *
 * THREAD SAFETY
 ...
 * (CabinManager.kt:2441) which AAOS already extrapolates. Dedup pushes to the Player
 ...
 *   Empty timeline + STATE_IDLE                            → switcher drops Cabin (regression)
...
class MediaSessionManager(
    ...
    private val artworkWriter: ((ByteArray) -> Uri?)? = null,
) {

    /** Routed transport controls from AAOS / steering wheel / cluster. Forwarded by
     *  [UsbAdapterPlayer.Callback] → this callback → CabinManager.sendKey(). */
    ...
    private val teyesClusterMediaBridge = TeyesClusterMediaBridge(context, logCallback)

    /** Publish safe text-only Honda/TEYES cluster guidance; no raw CAN or bitmap writes. */
    fun updateTeyesClusterNavigation(state: NavigationState) {
        teyesClusterMediaBridge.publishNavigation(state)
    }
    ...
    // lock. In practice CabinManager sets it once during initialize() and clears via
    ...
    private var artGeneration = 0L
    private var pendingArtGeneration: Long? = null
    private var latestMetadata: TrackMetadata? = null

    private data class TrackMetadata(
        val title: String?, val artist: String?, val album: String?, val appName: String?, val duration: Long,
    )
    ...
    /**
     * [MediaLibrarySession.Callback] for AAOS Media Center browse-tree queries. cabin_native
    ...
    private val libraryCallback = object : MediaLibrarySession.Callback {
        override fun onGetLibraryRoot(
            ...
            // lists Cabin with the expected card presentation even though the browse tree
        ...
        override fun onGetChildren(
            ...
            val infoItem = MediaItem.Builder()
                ...
                .setMediaMetadata(
                    MediaMetadata.Builder()
                        .setTitle(context.getString(com.cabin.R.string.app_status_browse_title))
                        .setSubtitle(context.getString(com.cabin.R.string.app_status_browse_description))
        ...
        override fun onConnect(
            ...
            // "cabin doesn't appear in source X" reports — the observer either binds
         ...
         * GMAudioServer.GMMediaKeyService.handleSWCKeys). Cabin registers as a
            ...
            if (mediaItems.isNotEmpty()) {
                ...
                Log.w(
                    ...
                    "[MEDIA_SESSION] onAddMediaItems from ${controller.packageName} uid=${controller.uid} " +
                        ...
                        "cabin is a projection mirror with no library; rejecting",
                )
            }
     ...
     * synchronously so CabinManager's sendKey() sees main-thread callbacks as before.
     */
     ...
     * CabinManager.initialize() on the main thread during plugin attachment.
    ...
    fun initialize() {
        ...
        try {
            ...
            // this manager is created by CabinManager with an application Context, not by
            ...
            // the lifecycle — AAOS observers see Cabin as a present, ready-but-paused
            ...
            // Log then rethrow so CabinManager's outer try/catch observes the failure.
            ...
            // CabinMediaBrowserService.onGetSession → getMediaLibrarySession() returns null
    ...
    fun release() {
        try {
            ...
            synchronized(sessionLock) {
                ...
                teyesClusterMediaBridge.release()
                player?.release()
    ...
    /** Register the transport-control sink (CabinManager routes to USB commands). */
    ...
    fun updateMetadata(
        ...
        synchronized(sessionLock) {
            ...
            latestMetadata = TrackMetadata(title, artist, album, appName, duration)
            ...
            if (albumArt != null && albumArt.isNotEmpty() && !albumArt.contentEquals(lastArtBytes)) {
                artGeneration++
                lastArtJob?.cancel()
                pendingArtGeneration = null
                lastArtBytes = albumArt.copyOf()
                lastArtUri?.let { revokeUriFromConsumers(it) }
                lastArtUri = null
            }
            val artBytesForNow = lastArtBytes
            val artUriForNow = lastArtUri
            ...
            // One URI write per artwork generation. Text-only updates retain the
            // pending artwork but its completion always publishes the latest text.
            // Cancellation cannot retract Handler callbacks, so validate the generation
            // after the IO write and again at the actual main-thread publication.
            if (artBytesForNow != null && lastArtUri == null && pendingArtGeneration != artGeneration) {
                val generation = artGeneration
                pendingArtGeneration = generation
                val bytesCapture = artBytesForNow
                lastArtJob = scope.launch {
                    val uri = try {
                        if (artworkWriter != null) artworkWriter.invoke(bytesCapture) else albumArtCache.put(bytesCapture)
                    } catch (e: Exception) {
                        log("[MEDIA_SESSION] AlbumArtCache.put failed: ${e.message}")
                        null
                    }
                    synchronized(sessionLock) {
                        if (generation != artGeneration) return@launch
                        if (uri == null) {
                            pendingArtGeneration = null
                            return@launch
                        }
                    ...
                    mainHandler.post {
                        synchronized(sessionLock) {
                            ...
                            val current = latestMetadata ?: return@synchronized
            ...
            log(
                "[MEDIA_SESSION] Metadata updated: $title - $artist " +
                    "(inlineBytes=${artBytesForNow != null}, uriPending=${pendingArtGeneration != null})",
            )
        }
    }
        ...
        synchronized(sessionLock) {
            try {
                val builder = MediaMetadata.Builder()
                    .setTitle(title ?: context.getString(com.cabin.R.string.app_status_unknown_media))
                    ...
                    .setDisplayTitle(title ?: context.getString(com.cabin.R.string.app_status_unknown_media))
                    .setArtist(artist ?: context.getString(com.cabin.R.string.app_status_unknown_media))
                    .setAlbumTitle(album ?: "")
                    .setSubtitle(appName ?: "Cabin")
                if (duration > 0) {
                ...
                teyesClusterMediaBridge.publishMetadata(title, artist, album, duration)
            } catch (e: Exception) {
    ...
    fun updatePlaybackState(playing: Boolean, position: Long = 0L) {
        synchronized(sessionLock) {
            ...
            try {
                ...
                teyesClusterMediaBridge.publishPlayback(playing, position)
     ...
     * source during this phase. Called on CabinManager's CONNECTING state.
     */
    /**
     * Mark the session ACTIVE — a phone is attached and projection is starting.
     *
     * Called on CabinManager's DEVICE_CONNECTED state (adapter PLUGGED event received
     ...
     *
     * Distinct from [setInactive]: this is the trigger that makes Cabin eligible for
     * homescreen Media card / cluster Media card binding. Without this transition,
     * AAOS consumers treat Cabin as not-a-valid-media-source and fall back to other
     * apps (Radio, etc.).
     *
     ...
     *
     * @param connectingPhase  true for CabinManager.State.CONNECTING (USB handshake
     *                         in progress); false for CabinManager.State.DEVICE_CONNECTED
    ...
    fun setProjectionActive(connectingPhase: Boolean) {
        synchronized(sessionLock) {
            ...
            try {
                ...
                // with the CONNECTING-time call site (CabinManager.updateMediaSessionState),
     ...
     * cabin session must therefore ALWAYS expose non-null metadata —
     ...
     * PlaybackState or session-active flags. Used by [CabinManager.setStatusText]
     ...
     *
     * Called on CabinManager's DISCONNECTED or CONNECTING state (app cold start,
     ...
     * CarMediaService/CarLauncher/ClusterMediaActivity treat Cabin as not a valid
    ...
    companion object {
        ...
        private const val SESSION_ID = "CabinMediaSession"
        ...
        private const val ROOT_ID = "cabin_root"
        ...
        private const val BROWSE_INFO_ITEM_ID = "cabin_browse_info"

        /**
         * Single live [MediaSessionManager] instance. Set by [initialize]; cleared by
         * [release]. Accessed by [CabinMediaBrowserService.onGetSession] to obtain the
         ...
         * comment in [initializeCabinManager] — revision 2026-04-23 static-analysis
         ...
         * [CabinMediaBrowserService.onCreate] so the session exists before any AAOS
        ...
        /** Release the singleton (called from [CabinMediaBrowserService.onDestroy]). */
        fun releaseInstance() {
