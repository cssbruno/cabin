...
private const val TAG = "CABIN_BROWSER"

/**
 * Cabin media service bridging the app to AAOS Media Center / steering-wheel controls.
 *
 * PURPOSE
 * - Registers the Cabin app as a selectable media source on Android Automotive OS
 ...
 * STATE_READY with `playWhenReady=true` (active playback). BUT cabin_native's
 ...
 * mechanisms call `startForeground` with compatible type masks. Manual reconciliation
 * reuses the posted media notification so it preserves Media3's playback controls.
 *
 ...
 * track loaded yet.
 */
class CabinMediaBrowserService : MediaLibraryService() {

    /**
     * Foreground-mode guard. See class-level KDoc for the race it prevents.
     */
    private var isForeground = false
    @Volatile private var manualForegroundRequested = false
    private val foregroundLock = Any()
    private val foregroundHandler = Handler(Looper.getMainLooper())
    private val foregroundReconcile =
        object : Runnable {
            override fun run() {
                if (!manualForegroundRequested) return
                if (!startForegroundMode() && !isForeground) stopSelf()
            }
        }
     ...
     * `CabinManager.sendKey(CommandMapping.PAUSE)` — which would emit a SPURIOUS USB
    ...
    override fun onTaskRemoved(rootIntent: Intent?) {
        if (CabinProjectionService.hasRegisteredProcessManager()) {
            // The widget foreground service still owns USB/audio. Keep this service and
            // its MediaSession token alive so AAOS controls/metadata survive task removal.
            if (BuildConfig.DEBUG) {
                Log.d(TAG, "[BROWSER_SERVICE] onTaskRemoved — projection service still owns session")
            }
            return
        }
        if (isForeground) {
    ...
    override fun onDestroy() {
        ...
        val retainForProjection = CabinProjectionService.hasRegisteredProcessManager()
        instance = null
        stopForegroundMode()
        // A headless projection may still be publishing metadata through this singleton.
        // Keep it until the projection service also stops; that service calls
        // releaseRetainedSessionIfServiceStopped() from its onDestroy path.
        if (!retainForProjection) MediaSessionManager.releaseInstance()
        super.onDestroy()
    }
    ...
    private fun createNotificationChannel() {
        val channel = NotificationChannel(
            ...
            localizedString(R.string.app_status_channel_title),
            NotificationManager.IMPORTANCE_LOW, // silent; adapter-connected indicator
        ).apply {
            description = localizedString(R.string.app_status_channel_description)
            setShowBadge(false)
    ...
    private fun buildNotification(): Notification {
        ...
        return NotificationCompat.Builder(this, NOTIFICATION_CHANNEL_ID)
            .setContentTitle(currentTitle ?: localizedString(R.string.app_name))
            .setContentText(currentArtist ?: localizedString(R.string.app_status_adapter_connected))
    ...
    private fun startForegroundMode(): Boolean {
        manualForegroundRequested = true
        var started = false
        synchronized(foregroundLock) {
            try {
                val notification = currentMediaNotification() ?: buildNotification()
                ...
                manualForegroundRequested = false
                Log.e(TAG, "[BROWSER_SERVICE] Failed to start foreground: ${e.message}")
            }
        }
        // Media3 may independently call stopForeground when Player state changes.
        // Reassert USB ownership with the existing media notification, preserving its
        // controls/artwork while repairing an isForeground flag Media3 cannot update.
        if (started) {
            foregroundHandler.removeCallbacks(foregroundReconcile)
            foregroundHandler.postDelayed(foregroundReconcile, FOREGROUND_RECONCILE_MS)
        }
        return started
    }
    ...
    private fun refreshNotification() {
        // Serialize with Media3's main-thread notification posts. USB metadata updates
        // must not replace MediaStyle with the plain connection-status notification.
        foregroundHandler.post {
            synchronized(foregroundLock) {
                if (!isForeground) return@post
                try {
                    if (currentMediaNotification() != null) return@post
    ...
    private fun currentMediaNotification(): Notification? =
        getSystemService(NotificationManager::class.java).activeNotifications
            .firstOrNull { it.id == NOTIFICATION_ID && it.tag == null }
            ?.notification
            ?.takeIf { it.extras?.containsKey(Notification.EXTRA_MEDIA_SESSION) == true }

    // ──────────────────────────────────────────────────────────────────────
    // Companion — static API for CabinManager (unchanged from legacy path)
    // ──────────────────────────────────────────────────────────────────────
    ...
    companion object {
        ...
        private const val FOREGROUND_RECONCILE_MS = 15_000L

        @Volatile private var instance: CabinMediaBrowserService? = null
         ...
         * CabinManager when MediaSessionManager publishes new metadata.
         *
         ...
         * CabinManager currently drives this from a single producer (USB read thread
        ...
        fun startConnectionForeground(context: Context) {
            val intent = Intent(context, CabinMediaBrowserService::class.java).apply {
                action = ACTION_START_FOREGROUND
            }
            try {
                context.startForegroundService(intent)
            } catch (error: RuntimeException) {
                // A background-start restriction must not abort the USB state machine.
                // CabinProjectionService remains the primary FGS for widget sessions.
                Log.e(TAG, "[BROWSER_SERVICE] Unable to request foreground start: ${error.message}")
            }
        }

        /**
         * Stop foreground mode. Called from CabinManager on DISCONNECTED state. The
        ...
        /** Release a session retained for headless projection after its owner exits. */
        fun releaseRetainedSessionIfServiceStopped() {
            if (Looper.myLooper() != Looper.getMainLooper()) {
                Handler(Looper.getMainLooper()).post { releaseRetainedSessionIfServiceStopped() }
                return
            }
            if (instance == null && !CabinProjectionService.hasRegisteredProcessManager()) {
