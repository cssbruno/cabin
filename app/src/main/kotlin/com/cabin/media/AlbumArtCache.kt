...
private const val TAG = "CABIN_ART_CACHE"
 ...
 * and blank the card. Writes are instrumented (CABIN_ART_CACHE logtag, via
 ...
 * - [put] serializes decode, writes and pruning across cache instances, including
 *   old managers finishing blocking IO after a replacement manager is created.
 *
...
class AlbumArtCache(
     ...
     */
    fun put(bytes: ByteArray, maxPx: Int = DEFAULT_MAX_PX): Uri? = synchronized(writeLock) {
        putLocked(bytes, maxPx)
    }

    private fun putLocked(bytes: ByteArray, maxPx: Int): Uri? {
        if (bytes.isEmpty()) {
