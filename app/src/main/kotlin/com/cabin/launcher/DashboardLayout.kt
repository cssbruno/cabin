...
/** Four columns by two rows. Reject overlap, overflow and duplicate live projection surfaces. */
internal fun validDashboard(layout: DashboardLayout): Boolean {
    ...
    for (tile in layout.tiles) {
        if (tile.id <= 0 || tile.page !in 0 until layout.pages || tile.width !in 1..4 || tile.height !in 1..2 ||
            tile.x < 0 || tile.y < 0 || tile.x + tile.width > 4 || tile.y + tile.height > 2 ||
...
class DashboardPreferences(context: Context, driver: Int, vehicle: Int, dialect: TeyesVehicleDataLayout) {
    ...
    private fun save(layout: DashboardLayout): Boolean {
        ...
        prefs.edit().putString(key, JSONObject().put("pages", layout.pages).put("tiles", tiles).toString()).apply()
    ...
    fun add(module: DashboardModule, page: Int, widgetId: Int = 0): Boolean {
        val tile = DashboardTile((state.value.tiles.maxOfOrNull { it.id } ?: 0) + 1, module, page, 0, 0,
            if (module in setOf(DashboardModule.PROJECTION, DashboardModule.WIDGET, DashboardModule.CLIMATE, DashboardModule.REAR_CLIMATE, DashboardModule.SEATS, DashboardModule.DEFROST)) 2 else 1,
            if (module in setOf(DashboardModule.PROJECTION, DashboardModule.WIDGET)) 2 else 1, widgetId)
        return place(tile, page)
    }
    private fun place(tile: DashboardTile, page: Int): Boolean {
        ...
        val positions = listOf(tile.x to tile.y) + (0..1).flatMap { y -> (0..3).map { x -> x to y } }
    ...
    private fun read(): DashboardLayout = try {
        ...
        if (raw == null || raw.length > 16384) DashboardLayout() else {
            val json = JSONObject(raw); val array = json.getJSONArray("tiles")
            require(array.length() <= 32)
            DashboardLayout(json.getInt("pages"), (0 until array.length()).map { i ->
                val t = array.getJSONObject(i)
                DashboardTile(t.getInt("id"), DashboardModule.valueOf(t.getString("kind")), t.getInt("page"), t.getInt("x"), t.getInt("y"), t.getInt("w"), t.getInt("h"), t.optInt("widget", 0))
