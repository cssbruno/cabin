 ...
 * CabinClusterSession for non-GM AAOS) so the Trip payload handed to
...
object TripBuilder {
    fun buildTrip(
        ...
        val measurementUnit = MeasurementPreferences.get(context).unit.value
        ...
        val stepEstimate =
            TravelEstimate
                .Builder(
                    DistanceFormatter.toDistance(state.remainDistance, measurementUnit),
                    eta,
                ).build()
        ...
        if (state.hasNextStep) {
            ...
            val nextStepEstimate =
                TravelEstimate
                    .Builder(
                        DistanceFormatter.toDistance(state.distanceToDestination, measurementUnit),
                        eta,
                    ).build()
