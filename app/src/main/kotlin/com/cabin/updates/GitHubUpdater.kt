...
internal data class UpdateStatus(val phase: UpdatePhase = UpdatePhase.IDLE, val release: UpdateRelease? = null, val progress: Int = 0)
internal class GitHubUpdater private constructor(private val context: Context) {
    ...
    suspend fun check() = withContext(Dispatchers.IO) { lock.withLock {
        ...
        mutable.value = old.copy(phase = UpdatePhase.CHECKING)
        try {
        ...
        catch (_: Exception) { mutable.value = old.copy(phase = UpdatePhase.FAILED) }
    } }
    ...
    suspend fun download() = withContext(Dispatchers.IO) { lock.withLock {
        ...
        try {
            ...
            readyFile.delete(); require(partial.renameTo(readyFile))
        ...
        catch (_: Exception) { mutable.value = UpdateStatus(UpdatePhase.FAILED, release) }
    ...
    private fun validateApk(file: File, release: UpdateRelease) {
        ...
        require(PackageInfoCompat.getLongVersionCode(candidate) == release.versionCode && release.versionCode > PackageInfoCompat.getLongVersionCode(current))
    ...
    suspend fun installIntent(): Intent = withContext(Dispatchers.IO) { lock.withLock {
        ...
        require(readyFile.length() == release.size)
        val digest = MessageDigest.getInstance("SHA-256")
