...
public class H264Renderer {
    ...
    private volatile boolean preferredDecoderFailed;
    ...
    private final AtomicBoolean overlayRecoveryPending = new AtomicBoolean(false);
    // Only guards the short producer copy/publication and staging handoff. Codec
    // allocation and worker joins must never hold this lock or block USB ingestion.
    private final Object stagingLock = new Object();
    private volatile boolean stagingReady;
    ...
    // 5 buffers: 1 write + up to 3 in queue + 1 feeder. Avoid a full-frame spare:
    // at 4K that unused margin alone costs ~6 MiB and increases low-memory kills.
    private static final int MIN_STAGED_FRAME_CAPACITY = 2 * 1024 * 1024;
    private static final int MAX_STAGED_FRAME_CAPACITY = 8 * 1024 * 1024;
    private final int stagedFrameCapacity;
    private static final int STAGED_FRAME_COUNT = 5;               // write + queue(3) + feed
    ...
    public H264Renderer(int width, int height, Surface surface, LogCallback logCallback,
        ...
        long resolutionCapacity = ((long) width * (long) height * 3L) / 4L;
        this.stagedFrameCapacity = (int) Math.min(
                MAX_STAGED_FRAME_CAPACITY,
                Math.max(MIN_STAGED_FRAME_CAPACITY, resolutionCapacity));
    }
    ...
    private void startLocked() {
        ...
        try {
            ...
            if (mCodec != null && preferredDecoderName != null) {
                try {
                    if (preferredDecoderName.equals(mCodec.getName())) {
                        preferredDecoderFailed = true;
                        log("Preferred decoder failed to configure/start; next retry uses generic AVC decoder");
                    }
                } catch (Exception ignored) { /* codec may already be invalid */ }
            }
    ...
    private boolean canRecover() {
        return !stopped && running && mCodec != null && surface != null && surface.isValid();
    }
    ...
    private void requestRecoveryKeyframe() {
     ...
     *
     * Recovery runs on the codec executor and requests a fresh IDR through the
     * existing callback. Repeated requests are coalesced until recovery finishes.
     */
    public void flushCodec() {
        if (stopped || !running) return;
        final MediaCodec requestedCodec = mCodec;
        final Surface requestedSurface = surface;
        if (requestedCodec == null || requestedSurface == null ||
                !overlayRecoveryPending.compareAndSet(false, true)) return;
        // A bare MediaCodec.flush() leaves producer/feeder frames queued and preserves
        // syncAcquired, allowing stale P-frames to enter the freshly flushed codec.
        // reset() stops both worker threads, clears staging, resets the sync gate, and
        // requests a new IDR through the existing callback.
        try {
            executors.mediaCodec1().execute(() -> {
                try {
                    synchronized (codecLock) {
                        // A stop, surface swap, or another reset makes this queued
                        // request obsolete. Never restart a detached renderer.
                        if (!canRecover() || mCodec != requestedCodec || surface != requestedSurface) return;
                        log("[LIFECYCLE] Full codec reset for overlay recovery");
                        resetLocked();
                    }
                    requestRecoveryKeyframe();
                } finally {
                    overlayRecoveryPending.set(false);
                }
            });
        } catch (RejectedExecutionException e) {
            overlayRecoveryPending.set(false);
            log("[LIFECYCLE] Overlay recovery executor unavailable: " + e.getMessage());
        }
    }
    ...
    private void initCodec(int width, int height, Surface surface) throws IOException {
        ...
        if (preferredDecoderName != null && !preferredDecoderName.isEmpty() && !preferredDecoderFailed) {
            try {
    ...
    private void initStaging() {
        // Build the entire pool privately before allowing USB to claim any buffer.
        StagedFrame[] frames = new StagedFrame[STAGED_FRAME_COUNT];
        for (int i = 0; i < frames.length; i++) {
            frames[i] = new StagedFrame(stagedFrameCapacity);
        }
        synchronized (stagingLock) {
            ...
            // Publish the complete buffer set to the producer.
            writeFrame = frames[0];
            for (int i = 1; i < STAGED_FRAME_COUNT; i++) {
                framePool.offer(frames[i]);
            }
            stagingReady = true;
        }
    ...
    public boolean feedDirect(byte[] data, int offset, int length) {
        if (!running || !stagingReady) return false;
        framesReceived.incrementAndGet();
        sessionFramesReceived.incrementAndGet();
        logStats();

        // Guard: reject frames exceeding staging capacity (corrupted USB data)
        if (length > stagedFrameCapacity) {
            oversizedDropCount.incrementAndGet();
            logPerf("Oversized drop: " + length + "B > " + stagedFrameCapacity + "B");
            return false;
        }

        synchronized (stagingLock) {
            // Stop/reset may have begun after the fast check above.
            if (!running || !stagingReady) return false;
            ...
            if (stagingOffer(wf)) {
                ...
                // The next feedDirect() retries the pool before dropping the incoming frame.
            } else {
